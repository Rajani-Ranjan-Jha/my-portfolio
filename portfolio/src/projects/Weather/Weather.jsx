    ...
    <>
      <div className='w-full h-screen bg-gradient-to-r from-blue-400 via-violet-400 to-blue-500 flex flex-col items-center'>
        ...
        <div className="w-10/12 max-w-md flex flex-col justify-around items-center bg-white bg-opacity-30 backdrop-blur-md rounded-2xl p-6 shadow-lg">
          <div className="w-full md:w-8/10  flex justify-center items-center mb-6">
            <input
              type="text"
              placeholder='Enter the location'
              className='w-7/10 flex-grow p-2 shadow-md border border-indigo-600 rounded-l-xl focus:outline-none focus:ring-1 focus:ring-indigo-800'
              value={cityName}
              onChange={(e) => setCityName(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter') search() }}
            />
            <button
              className='w-3/10 p-2 bg-indigo-600 text-white rounded-r-xl shadow-md focus:ring-1 focus:ring-indigo-800'
              type='button'
              onClick={search}
            >
              Search
